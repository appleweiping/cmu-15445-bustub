...
void ExtendibleHTableDirectoryPage::Init(uint32_t max_depth) {
  ...
  global_depth_ = 0;
  // A depth-0 directory has a single slot; initialise it and all reachable slots.
...
auto ExtendibleHTableDirectoryPage::GetSplitImageIndex(uint32_t bucket_idx) const -> uint32_t {
  // The split image differs only in the highest bit of the local depth. Toggling
  // bit (local_depth - 1) of the index gives the sibling that shared the bucket.
...
auto ExtendibleHTableDirectoryPage::GetGlobalDepthMask() const -> uint32_t {
  return (global_depth_ == 0) ? 0U : ((1U << global_depth_) - 1);
}

auto ExtendibleHTableDirectoryPage::GetLocalDepthMask(uint32_t bucket_idx) const -> uint32_t {
  uint32_t local_depth = local_depths_[bucket_idx];
  return (local_depth == 0) ? 0U : ((1U << local_depth) - 1);
}
...
void ExtendibleHTableDirectoryPage::IncrGlobalDepth() {
  // Doubling the directory: copy each existing entry into its new high-bit twin.
...
auto ExtendibleHTableDirectoryPage::CanShrink() -> bool {
  // Can shrink only if every bucket's local depth is strictly less than global.
  if (global_depth_ == 0) {
    return false;
  }
