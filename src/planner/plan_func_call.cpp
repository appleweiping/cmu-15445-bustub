...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
    -> AbstractExpressionRef {
  // 1. Recognize the "lower" / "upper" scalar string functions.
  // 2. Each takes exactly one argument.
  // 3. Return a StringExpression wrapping that argument.
  if (func_name == "lower" || func_name == "upper") {
    if (args.size() != 1) {
      throw Exception(fmt::format("{} function should have exactly 1 argument", func_name));
    }
